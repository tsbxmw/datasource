...
func (as *AuthService) TokenGenerate(userId int) (key, secret string, err error) {
	...
	if err = models.DB.Where("user_id=?", userId).First(&authModel).Error; err != nil {
		...
		if err = models.DB.Table(authModel.TableName()).Create(&authModel).Error; err != nil {
			return
		}
	} else {
...
func (as *AuthService) RefreshToken(userId int)(key, secret string, err error) {
	...
	key = authModel.AppKey
	authModel.AppSecret = secret
	if err = models.DB.Table(authModel.TableName()).Update(&authModel).Error; err != nil {
		return
	}
	return
}